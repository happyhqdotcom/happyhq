...
ULTIMATE GOAL: A working local Next.js app where a user can teach an AI assistant a repeatable knowledge task through conversation, then delegate work that the assistant plans and executes autonomously — producing output the user would actually use. Consider missing elements and plan accordingly. If an element is missing, search first to confirm it doesn't exist, then if needed author the specification at specs/FILENAME.md. If you create a new element then document the plan to implement it in @IMPLEMENTATION_PLAN.md using a subagent.
