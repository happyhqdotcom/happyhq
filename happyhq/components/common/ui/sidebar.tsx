...
/** Like useSidebar but returns null when no SidebarProvider is present. */
    ...
    >
      ...
      <div
        className={cn(
          ...
          variant === 'floating' || variant === 'inset'
            ? 'pr-1 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+2px)]'
            : 'group-data-[collapsible=icon]:w-(--sidebar-width-icon)',
          className,
        )}
        {...props}
      >
        <div
          ...
          className={cn(
            ...
            variant === 'floating' && [
              'bg-sidebar',
              'group-data-[variant=floating]:rounded-t-2xl',
              'group-data-[variant=floating]:border',
              'group-data-[variant=floating]:border-sidebar-border',
              'group-data-[variant=floating]:shadow-sm',
            ],
            // Non-floating variants still get bg-sidebar + border
            variant !== 'floating' && [
              'bg-sidebar',
              side === 'left' && 'border-sidebar-border border-r',
              side === 'right' && 'border-sidebar-border border-l',
            ],
          )}
        >
          {children}
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={cn(
        'absolute top-4 bottom-0 z-20 hidden w-8 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-zinc-200/80 sm:flex',
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        'group/group-label text-sidebar-foreground/90 ring-sidebar-ring flex h-8 shrink-0 items-center rounded-md px-2 text-sm font-medium outline-hidden transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
