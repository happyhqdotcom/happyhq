...
4. The app creates the task directory structure at root (see [Filesystem Layout](filesystem-layout.md)):
   - `tasks/{task-slug}/task.md` — Frontmatter (title, `stream` field if the chat has a stream association) and the body holds `textContext` as the task description
   - `tasks/{task-slug}/inputs/` — User-provided files (`setupTaskFromChat()` moves files from `.chats/{sessionId}/uploads/` to `inputs/`)
...
2. **Server actions** (`createTask()` + `setupTaskFromChat()` in `lib/actions.ts`): `createTask()` creates `tasks/{slug}/task.md` with frontmatter (title, stream from chat's `streamSlug`) and `textContext` as the body — the task description planning will read. `setupTaskFromChat()` creates subdirectories (`inputs/`, `working/`, `outputs/`) and moves files listed in `CreateTask.files` from `.chats/{sessionId}/uploads/` to `inputs/` via `fs.rename` (see future direction note below).
