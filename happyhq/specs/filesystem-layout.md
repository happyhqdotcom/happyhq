...
~/HappyHQ/                              # Workspace root (= agent CWD)
  ...
  tasks/                                 # All tasks (flat — see Task List spec)
    {title-slug}-{timestamp}/            # One directory per task (time-sortable suffix)
      ...
      inputs/                            # User-provided files for this task
...
- **`inputs/`** — Files provided by the user for this specific task. Files are moved here as directories from `uploads/` by `setupTaskFromChat()` — each subdirectory contains `original.{ext}` and `raw.txt` (if available). Planning and working agents read `raw.txt` for text content instead of dealing with binary formats directly. The task description (brief, instructions, relevant details Q gathered during the learning conversation) lives in the body of `task.md`, not as a separate file. Legacy tasks may still have a loose `inputs/context.md` from before the task.md migration; the read layer falls back to it when `task.md` body is empty. See [Data Flow](data-flow.md).
...
- [x] `createTask` creates task.md with frontmatter (and body = description) at root `tasks/{slug}/`; `setupTaskFromChat` moves uploads from `.chats/{sessionId}/uploads/` to inputs/ (`lib/actions.test.ts`)
